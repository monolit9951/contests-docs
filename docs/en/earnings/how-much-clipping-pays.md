...
description: What clips pay comes down to three numbers - the rate, the views and the cap. Rates from $0.30 to $1.00 per 1000 views, a 2000-view threshold, a $50 cap per submission.
...
What a clipper earns comes out of one formula with three numbers in it: the rate per 1000 views, the views the clip gets, and the cap per submission. Rates across live contests run from **$0.30 to $1.00 per 1000 views** (median **$0.50**). The threshold in a typical live contest is **2000 views**: clear it and the submission joins the budget split. The cap per submission in typical contests is **$50**. The buyer sets all three when the contest is created and puts them on the card, so you can work out your figure before you open the editor.
...
Yes. The buyer locks the budget when creating the contest, and until the results are settled that money is protected: it cannot be pulled back. The platform acts as intermediary and guarantor so that neither side gets scammed. Whatever is left after the split goes back to the buyer.
