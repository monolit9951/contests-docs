...
Every DareBay contest uses **one of three** selection types. The type is
...
When the entry window closes, the system takes the top N most-liked
submissions and draws the winners at random from that pool.

- **Good for:** giveaways, contests with no expert judging, hype
  campaigns.
- **Feels like:** a lottery with a popularity filter.
- **What the participant does:** submits work that people will like; the
  draw decides the rest.
...
next in line (voting and top-liked contests) or the organizer picks again
