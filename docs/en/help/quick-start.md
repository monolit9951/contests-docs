---
title: "Quick start on DareBay: your first task from zero"
description: "Quick start on DareBay: pick one of three roles and go. Launch a contest, submit work, or watch and vote. One account covers the site and Telegram."
provenance: { snapshot_date: "2026-08-15", source: "darebay-prod" }
numbers_used: []
---

<div class="docs-cover">
  <div class="docs-cover-eyebrow">Quick start</div>
  <h1 class="docs-cover-title">Get going<br>in five minutes.</h1>
  <p class="docs-cover-lede">Three roles on one platform, pick yours. Minutes, not hours.</p>
</div>

# Quick start

A minute after you sign up you can launch a contest, send in work or vote,
depending on why you came. There are three roles: an **organizer** sets the
activity and the reward, a **participant** makes their own version of the
content and takes that reward, a **viewer** watches, comments and votes, and
often that vote decides it. Most accounts end up doing all three.

DareBay has two equal front doors and one account behind them: the site
[**darebay.com**](https://darebay.com) and Telegram (the app, the bot, the
group). Come in either way and you land in the same place: your activities, your
work and your wallet stay with you in both.

## If you want to launch a contest

Put up the prize, write the terms, publish. The budget is recorded on the platform
straight away, entries come in, a winner is picked, and the platform records the
credit or payout obligation with a status everyone can follow. In a wallet-backed
contest the reward reaches the winner's balance automatically.

→ [**Create your first contest**](/en/help/your-first-contest)

## If you want to take part

Browse the active contests and read the terms. There are two shapes of task, and
which one you are looking at changes everything about how you work:

- **Pay per view.** You publish the video on the site the contest names, send back
  the link, and the budget is split by counted views across everyone who cleared
  the view threshold. Several people get paid, not one.
- **Everything else.** You upload the work to DareBay (text, image or video) and
  wait for the selection stage, where a vote, the organizer or a seeded draw
  decides.

A wallet-backed prize was locked on the platform before you even showed up; a
manual-payout prize remains the organizer's direct payment obligation. Once a
result is in, the platform records the amount and status, and a wallet-backed
reward reaches your balance automatically. Request a withdrawal from 10 USDT and
the platform does the rest.

→ [**Submit your work**](/en/help/submit-your-work)

## If you just want to watch

Scroll the feed, like, comment and, in voting contests, vote. Viewers shape the
outcome without submitting a single entry.

→ [**Watch, vote, win**](/en/help/watch-vote-win)

## Read this before you start

- **[Prizes and payouts](/en/help/prizes-and-payouts)**: what people put up as a
  reward (money or an item), how the result is recorded and how the reward reaches
  you. The contest defines its reward method; the balance-withdrawal wizard
  separately shows available USDT and Telegram Stars options.
- **[Verification and trust](/en/help/verification)**: what the badge on an
  account means and why it matters to everyone dealing with it.
