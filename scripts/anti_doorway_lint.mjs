...
/**
 * Предохранитель «остановись и подумай», а не гейт качества: он считает смысловые страницы,
 * а не судит ни одну из них. 60 → 90 по решению фаундера 18.09.2026 — общий кап на число
 * смысловых страниц снят, потолок остаётся только как заметный стоп-сигнал.
 *
 * Гейтами качества остаются те, что смотрят на текст: DUP_THRESHOLD (копипаста),
 * PARAPHRASE_THRESHOLD (пересказ), FAQ_ECHO_LIMIT и HEADING_ECHO_LIMIT (эхо интента),
 * и CAP_NEW_PER_WAVE — сколько новых страниц входит одной волной. Ни один из них не тронут.
 *
 * Вместо капа на корпус действует правило живучести: страница, у которой на D+28 нет ни чтений
 * ИИ-ботами, ни показов, ни посадок, сливается в родительскую. Корпус ограничивает спрос,
 * измеренный постфактум, а не число, выбранное заранее.
 */
export const CAP_TOTAL = 90
